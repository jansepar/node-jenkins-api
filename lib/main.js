...
var init = exports.init = function(host, options) {
    ...
    return {
        ...
        stop_job: function(jobname, number, callback) {
            /*
            Stop a currently running job with the listed build number
             */
            request({method: 'POST', url: build_url(STOP, jobname, number)}, function(error, response, body) {
                if ( error || response.statusCode !== 200 ) {
                    callback(error || true, response);
                    return;
                }
