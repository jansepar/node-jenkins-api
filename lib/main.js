...
var NEWFOLDER = '/createItem/?name=%s&mode=%s&Submit=OK';

var DELETE_BUILD = '/job/%s/%s/doDelete';
var BUILD = '/job/%s/build' + API;
var BUILDWITHPARAMS = '/job/%s/buildWithParameters%s'; //TODO how to handle this?
var STOP_BUILD = '/job/%s/%s/stop' + API;
var BUILD_INFO = '/job/%s/%s' + API;

//var ALL_BUILDS = '/job/%s' + API + '?tree=allBuilds[id,timestamp,result,duration]';
...
var LIST_VIEW = '/view/%s' + API;
var NEW_VIEW = '/createView' + API;
var CONFIG_VIEW = '/view/%s/configSubmit'; // NOTE form encoded not called via /api/json, TODO fix this
var DELETE_VIEW = '/view/%s/doDelete' + API;
var VIEW_ADD_JOB = '/view/%s/addJobToView' + API;
var VIEW_REMOVE_JOB = '/view/%s/removeJobFromView' + API;

var LIST = API;
var NEWJOB = '/createItem' + API;
var JOBINFO = '/job/%s' + API;
var JOB_CONFIG = '/job/%s/config.xml' + API;
var JOB_OUTPUT = '/job/%s/consoleText' + API;
var JOB_BUILD_QUEUE = '/queue/item/%s/'+API;
var DELETE = '/job/%s/doDelete' + API;
var DISABLE = '/job/%s/disable' + API;
var ENABLE = '/job/%s/enable' + API;

var CANCEL_ITEM = '/queue/cancelItem?id=%s';
var QUEUE = '/queue' + API;
...
var init = exports.init = function(host, options, token) {
    ...
    return {
        ...
        build: function(jobName, customParams, callback) {
          // TODO better handle the params
          if (typeof customParams === 'function' || !customParams) {
              urlPattern = BUILD;
              callback = customParams;
              customParams = null;
          } else {
              urlPattern = BUILDWITHPARAMS;
          }

          doRequest({
            method: 'POST',
            urlPattern: [urlPattern, jobName],
            successStatusCodes: [201, 302],
          }, customParams, callback);
        },

        stop_build: function(jobName, buildNumber, customParams, callback) {
          [jobName, buildNumber, customParams, callback] = doArgs(arguments, ['string', 'string', ['object', {}], 'function']);
        ...
        job_output: function(jobName, buildName, customParams, callback) {
            [jobName, buildName, customParams, callback] = doArgs(arguments, ['string', 'string', ['object', {}], 'function']);
        ...
        all_builds: function(jobName, param, customParams, callback) {
            [jobName, param, customParams, callback] = doArgs(arguments, ['string', 'string', ['object', {}], 'function']);
        ...
        queue_item: function(queueNumber, customParams, callback) {
            [queueNumber, customParams, callback] = doArgs(arguments, ['string', ['object', {}], 'function']);
        ...
        cancel_item: function(itemId, customParams, callback) {
            [itemId, customParams, callback] = doArgs(arguments, ['string', ['object', {}], 'function']);
        ...
        get_build_id_from_queue_with_id: function(queueId, customParams, callback) {
          [queueId, customParams, callback] = doArgs(arguments, ['string', ['object', {}], 'function']);
        ...
        /* Get Job Queue status from the queue_id that is obtained in the job build phase!  */
        get_build_id_from_queue: function(queueId, customParams, callback) {
          [queueId, customParams, callback] = doArgs(arguments, ['string', ['object', {}], 'function']);
